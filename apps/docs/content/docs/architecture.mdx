---
title: 系统架构
description: 深入了解 Open Tunnl 的双平面架构设计。
icon: Network
---

## 架构概览

Open Tunnl 采用**双平面架构**，将流量处理与业务逻辑分离：
...
Open Tunnl 使用 **Virtual Bind** 替代传统的端口绑定：
...
sequenceDiagram
    ...
    SSH-->>Client: 分配 abc123.tunnl.example.com
...
传统 SSH 反向隧道会在服务器上绑定真实端口。Open Tunnl 通过子域名路由，实现：
...
sequenceDiagram
    ...
    participant Server as Open Tunnl
