---
title: 部署概述
description: Open Tunnl 生产环境部署架构与端口规划。
icon: Server
---

## 架构概览

Open Tunnl 采用 **Sidecar 模式**，由两个容器协同工作：
...
| `:8080` | HTTP Proxy | 公网（通过反向代理） | 处理 `*.tunnl.example.com` 请求 |
...
- [Docker Compose 部署](/docs/deployment/docker-compose) - 完整的部署配置
